...
- **Build backend**: `hatchling` (was: `hatchling` -- kept, but metadata modernized)
...
- `requirements.txt`
- `MANIFEST.in`
- Legacy `[tool.black]` and `flake8` config replaced by `[tool.ruff]`

If these reappear after a sync, delete them again. All configuration is in `pyproject.toml`.

### 3. Source Layout
- **Layout**: `src/` layout
- **Package moved**: `qf_lib/` -> `src/qf_lib/`
- **Import unchanged**: `import qf_lib` still works

If upstream adds files to the old location, move them to `src/qf_lib/`.
...
3. **Check source layout**: If upstream adds new modules to the old path, move them to `src/qf_lib/`.
4. **Re-lock**: Run `uv lock` to update `uv.lock` with any new/changed dependencies.
5. **Verify**: Run `uv sync && uv run python -c "import qf_lib" && uv run pytest` (if tests exist).

## Quick Commands

```bash
uv sync                                    # Install all deps
uv run python -c "import qf_lib"           # Verify import
